...
class UserBuilder {
    ...
    public function buildUser ($get, $post) {
        ...
        if (!$source) {
            ...
            return new User($source['topic'], $source['first-name'], $source['last-name']);
        } else {
            // Put your filter_input and/or htmlspecialchars call here.
            return new User($source['topic'], $source['first-name']);
        }
    }
    ...
    private function hasRequiredParameters($arr) {
        ...
        $has_topic      = array_key_exists($this->topic, $arr);

        return ($has_first_name && $has_topic);
    }
